...
bool check_mem(unsigned int test_no, std::vector<u32> &mem,
		unsigned int mem_pos, i32 value) {
	i32 result = static_cast<u32>(mem[mem_pos / 4]);
	if (result != value) {
		std::cout << "Test " << test_no << " failed: " << result << " != "
				<< value << std::endl;
		return false;
	}
...
bool check_mem_byte(unsigned int test_no, std::vector<u32> &mem,
		unsigned int mem_pos, u8 value) {
