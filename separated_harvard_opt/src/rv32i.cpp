...
void fetch_instruction (const u32 memory[MEMORY_SIZE / 2], u32 pc, i32 &instr, u1 &error) {
...
void execute_branch (u7 op_code, u3 funct3, u32 &result, u32 &pc, u1 &error, i32 imm, u32 source1, u32 source2,
                     u1 &write_reg) {
  switch (op_code) {
    case 0b110'1111:  // JAL
      result = pc + 4;
    ...
    case 0b1100011: {  // Branch
      ...
      switch (funct3) {
        ...
        case 0b001:  // BNE
          ...
          break;
        case 0b100:  // BLT
          condition = static_cast<i32>(source1) < static_cast<i32>(source2);
          break;
        case 0b101:  // BGE
          condition = static_cast<i32>(source1) >= static_cast<i32>(source2);
          break;
        case 0b110:  // BLTU
          condition = static_cast<u32>(source1) < static_cast<u32>(source2);
          break;
        case 0b111:  // BGEU
          condition = static_cast<u32>(source1) >= static_cast<u32>(source2);
          break;
...
void execute_load (u3 funct3, i32 imm, u32 &result, const u32 memory[MEMORY_SIZE / 2], u1 &error, u32 source1,
                   u1 &write_reg) {
  u32 pos = (source1 + imm) >> 2;
  u32 offset = ((source1 + imm) % 4) * 8;
  if (!(pos & 0x400'0000)) {
    Error(&error);
  } else {
    pos &= 0x03FF'FFFF;
...
void execute_store (const u32 memory[MEMORY_SIZE / 2], u3 funct3, i32 imm, u1 &error, u32 source1, u32 source2,
    ...
    case 0b0010011:  // Arithmetic Intermediate
      ...
      switch (funct3) {
        case 0b000:  // ADDI
          ...
          break;
        case 0b010:  // SLTI
          if (static_cast<i32>(source1) < imm) {
            result = 1;
          } else {
            result = 0;
          }
          break;
        case 0b011:  // SLTIU
          if (static_cast<u32>(source1) < imm) {
            result = 1;
          } else {
            result = 0;
          }
...
void do_process (const u32 instr_memory[MEMORY_SIZE / 2], u32 data_memory[MEMORY_SIZE / 2], u1 *error) {
  ...
  decode_fetch_operands(instr, op_code, funct3, funct7, funct12, source1, source2, rd, immediate, g_xreg);
  switch (op_code) {
    case 0b1101111:  // JAL
    case 0b1100111:  // JALR
    case 0b1100011:  // Bxxx
      execute_branch(op_code, funct3, result, g_pc, g_error, immediate, source1, source2, write_reg);
      break;
    case 0b0000011:
      execute_load(funct3, immediate, result, data_memory, g_error, source1, write_reg);
      g_pc += 4;
      break;
    case 0b0100011:
      execute_store(data_memory, funct3, immediate, g_error, source1, source2, write_mem, result, write_addr);
      g_pc += 4;
      break;
    default:
      execute_arithm(op_code, funct3, funct7, source1, source2, immediate, result, g_error, g_pc, write_reg);
      g_pc += 4;
      break;
  }
